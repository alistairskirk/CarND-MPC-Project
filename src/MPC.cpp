...
// Predefine our indices based on the number of vars and start points
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	// Using weights for all the cost values to smooth out the performance. Manually tuned on course. 
	...
	// Adapted From Udacity Course Lesson 19, Chp 9 Solution Mind the Line
	for (size_t t = 1; t < N; t++) {
		...
		// The original equations from the Udacity Course were only for a 1 degree poly. Adapting for 3rd degree
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Assume 6 elements in the state vector (x,y,psi,v,eCTE,epsi)
  // Then 6 * 10 + 2 * 9 = 78
  size_t n_vars = 6*N + 2*(N-1);
  // TODO: Set the number of constraints: delta: +25deg to -25deg, a: -1 to 1
  ...
  // Set limits for state variables
  ...
  // Here we set the range of values delta to [-25, 25] in radians
  for (size_t i = delta_start; i < a_start; i++) {
	  vars_lowerbound[i] = -0.436332;
	  vars_upperbound[i] = 0.436332;
  }
  // Here we set the range of throttle to -1 and 1
  ...
  // Need to set initial state of constraints lower/upper bound
  ...
  //std::cout << "preparing to solve" << std::endl;
  // solve the problem
  ...
  // Add future waypoints as the result in main is expecting the future x and y values
